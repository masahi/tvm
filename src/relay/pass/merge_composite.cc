...
class MergeCompositeWrapper : public ExprMutator {
 public:
  explicit MergeCompositeWrapper(const tvm::Map<std::string, Expr>& pattern_map) : pattern_map_(pattern_map) {}
  ...
  Expr ExtractPattern(const Var& pattern, const Expr& root, Map<std::string, Array<Expr>>& var_map) {
  ...
  Expr ExtractPattern(const Constant& pattern, const Expr& root, Map<std::string, Array<Expr>>& var_map) {
    return root;
  }
  ...
  Expr VisitExpr_(const CallNode* cn) {
    ...
    for (const auto& x : pattern_map_) {
      ...
      if (MatchPattern(pattern, call)) {
        ...
        Function new_func = FunctionNode::make(free_vars, extract, call->checked_type_, {}, Attrs());
